...
impl YieldDistributor {
    ...
    pub fn take_snapshot(env: Env, user: Address) {
        ...
        let user_index: i128 = env.storage().persistent().get(&StorageKey::UserYieldIndex(user.clone())).unwrap_or(0);
        
        if user_balance > 0 {
            let pending_delta = (user_balance * (global_index - user_index)) / SCALE;
            let mut current_pending: i128 = env.storage().persistent().get(&StorageKey::UserPendingYield(user.clone())).unwrap_or(0);
            current_pending += pending_delta;
            env.storage().persistent().set(&StorageKey::UserPendingYield(user.clone()), &current_pending);
        }
        
        env.storage().persistent().set(&StorageKey::UserYieldIndex(user), &global_index);
    }
    ...
    pub fn claim_yield(env: Env, user: Address) {
        ...
        let pending: i128 = env.storage().persistent().get(&StorageKey::UserPendingYield(user.clone())).unwrap_or(0);
        if pending > 0 {
            // Reset pending yield
            env.storage().persistent().set(&StorageKey::UserPendingYield(user.clone()), &0i128);
